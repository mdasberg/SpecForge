...
Where specifications come from. A repository holding specification files is connected through a
GitHub App installation, scanned before anything is committed to, imported, and kept in sync as the
repository moves — on pull request by default, which is the moment a team wants review. SpecForge
mirrors that repository and never writes content back to it; the single outbound write is the
review status it reports onto a pull request.
