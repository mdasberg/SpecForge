...
Understanding a change is half the review; arguing about it is the other half. A reviewer selects a
section — or a phrase inside it — and starts a thread there, so a disagreement attaches to the exact
text it is about instead of a comment box at the bottom of the page. Threads are resolvable, because
an unresolved conversation is what blocks an approval, and they remember which version they were
written against, because a specification keeps moving. Every comment also records whether a human or
an agent wrote it, as an API field rather than a UI convention, so no surface can present agent output
as a colleague's opinion.
