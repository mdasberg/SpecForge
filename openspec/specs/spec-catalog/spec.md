...
The everyday surface onto what has been imported: browsing hundreds of specifications grouped the
way the reader works, filters that combine, full-text search that names the section it matched, and
a rendered document with a section outline, deep links and every historical version. Reading a
specification is also the entry point to reviewing one, so the rendered document and its anchors
are built here and reused by the review screens.
