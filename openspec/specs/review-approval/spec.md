...
A review that anyone can declare finished is not a review. Approval here is a rule the project sets —
a minimum number of approvals, optionally including a role such as architect — evaluated against
verdicts that are always attached to the head version they were cast on, so a new push resets what
was approved instead of silently carrying it forward. The gate that lets a specification reach
Approved has three inputs: the rule, the unresolved blocking conversations, and the failed blocking
checks. It names every condition it refuses on, and it has no override, because an approval that can
be waived tells a later reader nothing. Agents may comment and fail checks; they never approve.
