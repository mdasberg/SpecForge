...
- Introduce the PostgreSQL schema with Liquibase migrations, plus a `compose.yml` that is the
  single definition of local PostgreSQL and Keycloak and the environment the integration tests run
  against.
...
- New `keycloak/realm-export.json` and a Keycloak service in `compose.yml`:
...
- Local development needs Docker for PostgreSQL and Keycloak. No user administration screens are built:
