...
public interface Forge {
    ...
    Optional<ForgeInstallationInfo> installation(String installationExternalId);

    /** Every file path on a ref, from which the connection's glob selects the specifications. */
    List<String> listFiles(String installationExternalId, ForgeRef ref);

    /** One file's content at a ref, with the commit that last touched it. */
    Optional<ForgeFile> readFile(String installationExternalId, ForgeRef ref, String path);

    /** The commit a branch currently points at. */
    Optional<String> headCommit(String installationExternalId, ForgeRef ref);

    /** The paths a pull request changes, so a sync only re-imports what actually moved. */
    List<String> changedFiles(String installationExternalId, String repositoryFullName, int pullRequestNumber);
